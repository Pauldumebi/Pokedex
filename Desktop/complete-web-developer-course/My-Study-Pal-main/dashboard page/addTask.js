...
//===========form validation empty input field
let inputField = document.querySelector(".first-question"),
...
function showAnswer() {
  ...
  ansQ2.textContent = secondAnswer + time();
  ansQ3.textContent = thirdAnswer + thirdtime();
}
function showError(){                
...
//=============== Time function ===============
function time () {
  secQuestion = secondQuestion.value
  var [h,m] = secQuestion.split(":");
  return ((h%12+12*(h%12==0))+":"+m, h >= 12 ? 'PM' : 'AM');
}
function thirdtime () {
  trdQuestion = thirdQuestion.value
  var [h,m] = trdQuestion.split(":");
  return ((h%12+12*(h%12==0))+":"+m, h >= 12 ? 'PM' : 'AM');
}
